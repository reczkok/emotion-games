...
public class SelectionOutlineController : MonoBehaviour
{
    ...
    void Update()
    {
            ...
            RaycastHit hit;
            Debug.DrawLine(ray.origin, (ray.origin + ray.direction * RaycastDistance), Color.blue);
            if (Physics.Raycast(ray.origin, ray.direction, out hit,RaycastDistance))
            {

                var interactable = hit.transform.GetComponent<Interactable>();
                if (hit.transform.GetComponent<Renderer>() != null &&  interactable != null && interactable.IsInteractable())
                {
        ...
        if(TargetRenderer != null && Input.GetMouseButtonDown(0))
        {
            TargetRenderer.GetComponent<Interactable>().OnAction();
        }
    }
}
